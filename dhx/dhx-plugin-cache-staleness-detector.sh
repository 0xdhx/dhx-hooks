...
#   2.1.281 (Claude Code)   → literal CC version captured from 10.1-D-01-RESULT.md
emit_advisory() {
  local cache_path=$1
  local cache_mtime=$2
  local live_mtime=$3
  case "WARN" in
    REJECT)
      ...
      ;;
    WARN)
      echo "dhx-plugin-cache-staleness: WARN: $cache_path mtime=$cache_mtime older than live mtime=$live_mtime (informational on CC 2.1.281 (Claude Code) per empirical probe; run \`claude plugin install\` to refresh)" >&2
      ;;
    WARN_INCONCLUSIVE)
