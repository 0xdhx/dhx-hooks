...
| `probe-sync-mirror-publish-gate.sh` | yes | runs `sync-public-mirror.sh` end-to-end, but the ONLY push target is a mktemp **bare repo** passed via the `PUBLIC_REMOTE` env override — it never contacts `git@github.com:0xdhx/dhx-hooks.git`, and the rehearsal cases push nothing at all. Clones/builds stay under a mktemp dir removed on EXIT; the source repo is read-only. **If the `PUBLIC_REMOTE` override is ever removed, this probe must be reworked — never "fixed" by pointing it at the real remote.** (2026-07-21 publish-gate incident, 19 assertions) |
