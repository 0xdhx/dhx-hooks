...
| `probe-dashboard-notify-wiring.sh` | yes | read-only grep of the in-repo dispatcher for the dhx-dashboard notify wiring; conditional behavioral smoke runs `dhx-dashboard.cjs notify`/`count`, which only READ feeds + write to stdout (never the `variant-*.md` files — that is the separate `render` subcommand); output captured to a `mktemp` dir; never mutates live state (2026-06-04 dhx-dashboard notify SessionStart wiring probe) |
