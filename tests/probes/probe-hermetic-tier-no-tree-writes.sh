...
#
# Assertion 2 is the one that matters over time: it is CLASS parity, not a
# two-probe check. A third arming-latch probe added later without the guard reds
# here rather than re-creating the 2026-09-17 commit block on some future day.
#
...
while IFS= read -r p; do
  ...
  line=$(sed -n "${disc}p" "$p")
  chk "[2] $base — discriminator honours DHX_PROBE_HERMETIC" \
...
# Behavioural, against the real probes in a throwaway repo root. Arm BOTH latches
# under a sandbox XDG_RUNTIME_DIR, then assert neither probe writes into the
# corpus tree it would have written to.
GR="$TMPROOT/gaterepo"
XRD="$TMPROOT/xdg"
mkdir -p "$GR/tests/probes" "$XRD/dhx-statusline-stdin-probe" "$XRD/dhx-subagent-stop-sync-probe"
chmod 700 "$XRD/dhx-subagent-stop-sync-probe"
git -C "$GR" init -q 2>/dev/null
cp "$REPO/tests/probes/probe-effort-level-stdin-absent.sh" \
   "$REPO/tests/probes/probe-subagent-stop-sync.sh" "$GR/tests/probes/"
...
( cd "$GR" && XDG_RUNTIME_DIR="$XRD" DHX_PROBE_HERMETIC=1 \
    timeout 20 bash tests/probes/probe-effort-level-stdin-absent.sh ) >/dev/null 2>&1
EFF_RC=$?
( cd "$GR" && XDG_RUNTIME_DIR="$XRD" DHX_PROBE_HERMETIC=1 \
    timeout 20 bash tests/probes/probe-subagent-stop-sync.sh ) >/dev/null 2>&1

WROTE=$(find "$GR/tests/probes/.results" -type f 2>/dev/null | wc -l)
chk "[3a] armed latches + marker: NOTHING written under .results/" \
    "$([ "$WROTE" -eq 0 ] && echo yes || echo no)"
chk "[3b] the effort probe still RUNS its fixtures and passes (coverage kept, not dropped)" \
    "$([ "$EFF_RC" -eq 0 ] && echo yes || echo no)"

# 3c — POSITIVE CONTROL. Without the marker the armed latch must reach a
# DIFFERENT path, otherwise 3a proves nothing about the marker — it would pass
# just as happily if the latch were broken, or the arming dir misspelled, or the
# probe a no-op. Measured: the unmarked run walks past the discriminator into the
