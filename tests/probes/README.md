...
| `probe-dashboard-notify-wiring.sh` | decisions.md 2026-06-04 dhx-dashboard notify SessionStart wiring row (dispatcher invokes cross-repo `dhx-dashboard.cjs notify` via the `~/.claude/dhx-tools/` indirection behind `[ -e ]`/`< /dev/null`/`\|\| true`, stdout NOT redirected so the badge OSC reaches the pane, no `~/repos/cross-repo/` coupling; behavioral smoke asserts the `dhx_inbox_open` badge byte-shape, badge-payload == `count`, tmux-passthrough vs bare OSC branch, the one-name invariant (no `dhx_actionable`), and the silent-when-0 / labeled-distinct vitals banner) | `bash tests/probes/probe-dashboard-notify-wiring.sh` |
