...
> **`probe-effort-level-stdin-absent.sh` left this family on 2026-09-18** and is deliberately absent from the table above. Its watchdog premise — that CC's stdin payload carries no `effort` key — died 2026-05-13, and it spent four months reporting that into `[SUPERSESSION OBSERVED]`, where nothing surfaces it. It is now a **Convention B** (`exit_0_means_pass`) regression guard asserting that the effort level CC publishes is one `EFFORT_RENDER` can actually render; on an absent level it emits `skipped` rather than a verdict, because one armed capture cannot separate a dropped key from a transient miss. Its cells up to 2.1.275 in the corpus table below are the **unchanged historical record** and are not re-run or rewritten — the corpus is immutable evidence, and `supersession_found_drop_p3` is what those runs actually concluded; from 2.1.276 on, the same row carries the guard's `validated_stable` / `regression_found_*` cells under the new convention. The new `regression_found_*` token it introduces is the shared taxonomy's first decisive-negative. See `docs/decisions.md` 2026-09-18.

**Cross-version corpus state (per-probe × per-CC-version):**

| Probe | CC 2.1.121 (v1.2 baseline) | CC 2.1.140 (v1.3 Phase 15) | CC 2.1.145 (v1.3 Phase 18) | CC 2.1.148 (v1.4 Phase 19) | CC 2.1.150 (v1.4 — live 2026-05-26) | CC 2.1.272 – 2.1.278 (2026-09-15 → 09-19; adoption runs) |
|-------|---------------------------|---------------------------|---------------------------|---------------------------|-------------------------------------|-------------------------------------|
| `probe-effort-level-stdin-absent.sh` | `supersession_found_drop_p3` (`v1.2-phase-0/`) | `supersession_found_drop_p3` (`v1.3-multi-cc-ver/2.1.140/`) | — (not re-run) | — (not re-run) | — (not re-run; no-key probe, not in the 2026-05-26 live batch) | 2.1.273 + 2.1.275: `supersession_found_drop_p3` — the last two cells written by the watchdog form (dead premise, see the call-out above). 2.1.276 + 2.1.278: **`validated_stable`** — post-inversion Convention B: `level=high` renderable, `payload_version=host_version`, armed capture by the probe session (`v1.3-multi-cc-ver/2.1.27x/`) |
| `probe-installed-plugins-no-natural-heal.sh` | `supersession_found_drop_heal` (`v1.2-phase-0/`) | `v1_2_work_warranted` (`v1.3-multi-cc-ver/2.1.140/`) — **flipped** | — (not re-run) | `ambiguous` — `cell_outcome=auth_failure`; Cell 2 needs `ANTHROPIC_API_KEY` (unset) (`v1.3-multi-cc-ver/2.1.148/`) | `v1_2_work_warranted` — `cell_outcome=clean_no_heal`; decisively authed via `ANTHROPIC_API_KEY` (no 2.1.148-style `auth_failure`) (`v1.3-multi-cc-ver/2.1.150/`) | 2.1.275: `ambiguous` — `cell_outcome=setup_failure`, `cell1_rc=127`, **no producing binary** (see the 2.1.27x call-out). 2.1.276 + 2.1.278: `v1_2_work_warranted` — `cell_outcome=clean_no_heal`, both cells rc 0 |
| `probe-installed-plugins-badjson-natural-heal.sh` | `supersession_found_drop_heal` (`v1.2-phase-6/`) | `ambiguous` — the binary ran (`cell1_rc=0`) and observed `cell_outcome=badjson_no_heal`; the conclusion was downgraded by the probe's single-anchor `EXPECTED_CC_VERSION="2.1.121"` (allow-list, retired 2026-05-26) (`v1.3-multi-cc-ver/2.1.140/`) | `v1_2_work_warranted` — `cell_outcome=badjson_no_heal` (`v1.3-multi-cc-ver/2.1.145/`) | `ambiguous` — substantive `cell_outcome=badjson_no_heal` (work warranted); conclusion `ambiguous` only because 2.1.148 not in the probe's stale `cc_version` allow-list (`v1.3-multi-cc-ver/2.1.148/`) | `v1_2_work_warranted` — `cell_outcome=badjson_no_heal`, `confidence=HIGH` (`v1.3-multi-cc-ver/2.1.150/`) | 2.1.275: `ambiguous` — `cell_outcome=setup_failure`, `cell1_rc=127`, **no producing binary**. 2.1.276 + 2.1.278: `v1_2_work_warranted` — `cell_outcome=badjson_no_heal`, `confidence=HIGH` |
| `probe-installed-plugins-uninstalled-dhx-natural-heal.sh` | `supersession_found_drop_heal` (`v1.2-phase-6/`) | `ambiguous` — the binary ran (`cell1_rc=0`) and observed `cell_outcome=uninstalled_hn_heals`; the conclusion was downgraded by the probe's single-anchor `EXPECTED_CC_VERSION="2.1.121"` (allow-list, retired 2026-05-26) (`v1.3-multi-cc-ver/2.1.140/`) | `v1_2_work_warranted` (`v1.3-multi-cc-ver/2.1.145/`) | `ambiguous` — substantive `cell_outcome=uninstalled_hn_heals` (Hn() rehydrated dhx + preserved fakes — a HEAL signal); conclusion `ambiguous` only because 2.1.148 not in the probe's stale `cc_version` allow-list (`v1.3-multi-cc-ver/2.1.148/`) | `supersession_found_drop_heal` — `cell_outcome=uninstalled_hn_heals` (Hn() rehydrated dhx + preserved fakes), `confidence=HIGH` (`v1.3-multi-cc-ver/2.1.150/`) | 2.1.275: `ambiguous` — `cell_outcome=setup_failure`, `cell1_rc=127`, **no producing binary**. 2.1.276 + 2.1.278: `supersession_found_drop_heal` — `cell_outcome=uninstalled_hn_heals` (Hn() rehydrated dhx + preserved fakes), `confidence=HIGH` — now unanimous 2.1.140 → 2.1.278 |
| `probe-known-marketplaces-natural-heal.sh` | `v1_2_work_warranted` (`v1.2-phase-6/`) | `v1_2_work_warranted` (`v1.3-multi-cc-ver/2.1.140/`) | `v1_2_work_warranted` — `cell_outcome=km_no_heal` (`v1.3-multi-cc-ver/2.1.145/`) | `ambiguous` — substantive `cell_outcome=km_no_heal` (work warranted — km path NOT healed); conclusion `ambiguous` only because 2.1.148 not in the probe's stale `cc_version` allow-list (`v1.3-multi-cc-ver/2.1.148/`) | `v1_2_work_warranted` — `cell_outcome=km_no_heal`, `confidence=HIGH` (`v1.3-multi-cc-ver/2.1.150/`) | 2.1.272, 2.1.273, 2.1.275, 2.1.278: `v1_2_work_warranted` — `cell_outcome=km_no_heal`, `confidence=HIGH` (post-rewrite probe, `--binary`-pinned; no 2.1.276 cell — that run did not include km) |
...
> **2.1.272 – 2.1.278 column (added 2026-09-19):** one column for the five adoption-run versions, because within it only two things move and both are probe-side, not CC-side. **(1) The 2.1.275 `setup_failure` cells are not measurements.** The three `installed-plugins` probes swap `$HOME` to a sandbox, and since `~/.local/capbin/claude` → `claude-capped.sh` landed on PATH (2026-08-07) that wrapper re-resolves the real binary through `$HOME/.local/bin/claude` and exits **127** when it is not there — so from 2026-08-07 until `994e2036` (2026-09-18, `lib/resolve-cc-binary.sh`: one binary, resolved before the swap, invoked by absolute path) every cell those three wrote had launched nothing. 2.1.275 is the only column in this table inside that window; the 2.1.140 / 2.1.145 / 2.1.148 / 2.1.150 cells all predate capbin and all carry `cell1_rc=0`, which is why their `ambiguous` conclusions are allow-list downgrades of a real observation (cells say so: substantive `cell_outcome`, `cc_version_match=false`, `LOW`) and the 2.1.275 ones are `no_producing_binary` (the `binary_attribution` field, added `994e2036`, splits the two eras by evidence). No promotion count was ever credited from a non-answer cell: the N≥3 promotion executed 2026-05-26 on 2.1.121 / 2.1.140 / 2.1.145 / 2.1.148 — every one pre-capbin, rc 0. **(2) The effort probe changed convention mid-column** (inverted 2026-09-18): 2.1.273 / 2.1.275 are its last watchdog cells and 2.1.276 / 2.1.278 its first renderability-guard cells; the token flip is the inversion, not a CC change (`payload_version=host_version` on both post-inversion cells). Everything else in the column agrees with 2.1.150: `badjson_no_heal`, `clean_no_heal`, `km_no_heal` (km cells taken by the rewritten, `--binary`-pinned probe at 2.1.272 / 273 / 275 / 278), and `uninstalled_hn_heals` unanimous 2.1.140 → 2.1.278. **`setup_failure` token ruling (`docs/decisions.md` 2026-09-19):** it stays under `conclusion: ambiguous` — `cell_outcome=setup_failure` + `binary_attribution=no_producing_binary` already say "not measured, and why", and a new top-level token would need admitting to three consumers for a state the validator already refuses to let anyone cite as stable (assertion 5). `verify-multi-cc-results.sh` **prints** a `NOTE` line for each `setup_failure` cell it scans and never refuses on it: the cell is the audit trail that a run happened and could not launch, which is the thing a refusal would push someone to delete.
...
| `probe-empirical-arm-oracle.sh` | decisions.md 2026-09-19 "the D-01 arm needs credentials" row (the arm's verdict oracle in `lib/empirical-arm-classify.sh`: REFUTE needs ≥1 Stop-dispatch line; the credential gate; § 4 is the pre-fix rule as negative control) + the 2026-09-24 re-stamp-only-on-a-verdict-change row (§ 11: `arm_restamp_owed`, the `write-result` guard through the `CC_D01_RESULT_ARTIFACT` seam, the arm's Step 9) | `bash tests/probes/probe-empirical-arm-oracle.sh` |
...
| `probe-cc-matcher-routing-fingerprint.sh` | decisions.md 2026-09-23 matcher-routing-fingerprint row + backlog `2026-08-24-disjointness-probe-enumeration-residuals` (2026-09-19 ruling). `LIVE_RUNTIME: yes`: the normalized sha of CC's five-function hook-matcher routing cluster — resolved by literal anchor with a router-calls-splitter-and-alias coherence gate, never by minified name — equals the row last accepted in `config/cc-matcher-routing.tsv`; a changed sha is the cue to re-derive `BASH_MATCHER_FILTER`. Positive controls: the 2.1.273 caller token re-applied reproduces the recorded 2.1.273 sha; a one-token router mutant differs; a same-named decoy (the real 2.1.278 `H2e` collision) is not picked. Section D drives the live cell's own red/skip/record paths on synthetic version dirs. Mutation-measured 2026-09-23: coherence gate off, identity normalizer, inverted live compare, vacant-live-as-pass — 4/4 caught by name, each one hunk from a green NULL | `bash tests/probes/probe-cc-matcher-routing-fingerprint.sh` (`--record` to accept the newest build) |
