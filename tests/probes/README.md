...
| `probe-cc-version-observer-wiring.sh` | decisions.md 2026-09-17 cc-version-observer SessionStart wiring row (dispatcher invokes the cross-repo observer via the `~/.claude/dhx-tools/` indirection behind `[ -e ]`/`< /dev/null`/`|| true`, under `_dhx_child`, with stdout NOT redirected — the notice is the deliverable; behavioral smoke T1-T5 via override seams, incl. T3b asserting the notice names a keyed brief and skips a decoy) | `bash tests/probes/probe-cc-version-observer-wiring.sh` |
