...
| `probe-dirty-tree-attribution-gate.sh` | decisions.md 2026-07-28 dirty-tree attribution enrichment row (allowlist gate: non-allowlisted repo = byte-identical bare line + zero helper invocations; fail-open wrapper: absent/hung/malformed/wrong-version/oversize each degrade byte-identical; exit-3 canary = the one loud degrade; payload replaces bare line, `--repo`+`--self` argv; wording guard: no banned action verb / relative-age token) | `bash tests/probes/probe-dirty-tree-attribution-gate.sh` |
